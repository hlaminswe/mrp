...
class pos_order(osv.osv):
    ...
    def _amount_all(self, cr, uid, ids, name, args, context=None):
        ...
        for order in self.browse(cr, uid, ids, context=context):
            ...
            default_currency = res_users_obj.browse(cr, uid,
                uid, context=context).company_id.currency_id
            
            for payment in order.statement_ids:
                paymentamount=payment.amount
                if abs(default_currency.id - payment.currency_id.id)>0:
                    paymentamount=res_currency_obj.compute(cr,uid, payment.currency_id.id, default_currency.id,payment.amount,round=False, context=context)
                
                res[order.id]['amount_paid'] += paymentamount
                res[order.id]['amount_return'] += (paymentamount < 0 and paymentamount or 0)
        
            for line in order.lines:
                val1 += line.price_subtotal_incl
                val2 += line.price_subtotal
            res[order.id]['amount_tax'] = cur_obj.round(cr, uid, default_currency, val1-val2)
            res[order.id]['amount_total'] = cur_obj.round(cr, uid, default_currency, val1)
            #_logger.info("Resources %r", res);
        return res
    ...
    def add_payment(self, cr, uid, order_id, data, context=None):
        ...
        if 'currency'  in data.keys():
            args['currency_id'] = data['currency'][0]
        if 'currency_id'  in data.keys():
            args['currency_id'] = data['currency_id']
